      ...
      <c:forEach var="item" items="${list}" varStatus="loop">
             <li> 
		        ...
		        <c:if test="${not empty item.URLWebsite}">
	          		...
	          		<span><a href="${item.URLWebsite}" taregt="_blank">${item.URLWebsite}</a></span>
		        </c:if>
		       </p>
	            	</c:when>
			    ...
			    <div class="clr"></div>
			    <c:if test="${item.group == 'GUIDED_TOUR'}">
			    	<a href="<wa:url URI="${site}/visitBooked/displayForm/id/${item.id}"/>"><wa:mls>Participer ></wa:mls></a>
			    </c:if>
			    <div class="clr"></div>
	           </div>
	          <div class="btn">
