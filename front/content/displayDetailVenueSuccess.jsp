...
<div id="content_main">
	<c:if test="${not empty adminUser}">
		...
		<a href="${context}/admin/contentVenue/list.do?s_publishContent_id=${bean.id}&s_submit=Search"
		...
		<div id="box_right_information">
			<div id="feature_pratical_information">
				...
				<a href="http://www.addthis.com/bookmark.php?v=250&amp;username=wanabe" class="addthis_button share"
				   addthis:url="${config['URL']}<wa:url URI="${site}/content/displayDetailVenue/id/${bean.id}"/>"
				...
				<a href="javascript:addToMeetingPlanner('VENUE','${bean.id}');"><img src="${staticSite}/img/ml/btn_add_big.gif"
				                                                                     alt="<wa:mls>Add</wa:mls>" id="logo_add"/></a>
			...
			<div id="pratical_information">
				...
				<c:if
					test="${contentType ne 'ContentArticle' and not empty bean.latitude and bean.latitude != 0.0 and not empty bean.longitude and bean.longitude != 0.0}">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" id="venues_table">
		...
		<c:forEach items="${bean.contentRooms}" var="item" varStatus="loop">
			<tr class="row${loop.count mod 2}">
				...
				<td><c:choose><c:when test="${item.surface > 0}"><fmt:formatNumber value="${item.surface}" pattern="#######"/>
					<wa:mls>m&sup2;</wa:mls></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${item.height > 0}"><fmt:formatNumber value="${item.height}"
				                                                                  pattern="#####.##"/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${item.theater > 0}"><fmt:formatNumber value="${item.theater}"
				                                                                   pattern="#######"/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${item.school > 0}"><fmt:formatNumber value="${item.school}"
				                                                                  pattern="#######"/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				...
				<td><c:choose><c:when test="${item.banque > 0}"><fmt:formatNumber value="${item.banque}"
				                                                                  pattern="#######"/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${item.auditorium > 0}"><fmt:formatNumber value="${item.auditorium}"
				                                                                      pattern="#######"/></c:when><c:otherwise>-</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${item.interprateCabines == 0}"><wa:mls>On demand</wa:mls></c:when><c:when
					test="${item.interprateCabines == -1}"><wa:mls>No</wa:mls></c:when><c:otherwise><wa:mls>yes</wa:mls></c:otherwise></c:choose></td>
			</tr>
		</c:forEach>
	</table>
