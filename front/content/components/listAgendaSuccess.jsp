...
<!--tab:start-->
   ...
   <div class="TabbedPanelsContentGroup">
       <ul class="TabbedPanels">
        <li class="<c:if test="${panelGroup == 'HIGHLIGHT'}">TabbedPanelsTabSelected</c:if><c:if test="${panelGroup != 'HIGHLIGHT'}">TabbedPanelsTab</c:if>"><div><a href="<wa:url URI="${site}/content/displayAgenda/group/HIGHLIGHT"/><c:if test="${not empty waParam.caldate}">?caldate=${waParam.caldate}</c:if>"><wa:mls>A la une</wa:mls></a></div></li>
        <li class="<c:if test="${panelGroup == 'CULTURE'}">TabbedPanelsTabSelected</c:if><c:if test="${panelGroup != 'CULTURE'}">TabbedPanelsTab</c:if>"><div><a href="<wa:url URI="${site}/content/displayAgenda/group/CULTURE"/><c:if test="${not empty waParam.caldate}">?caldate=${waParam.caldate}</c:if>"><wa:mls>ALL</wa:mls></a></div></li>
        <li class="<c:if test="${panelGroup == 'GO_OUT'}">TabbedPanelsTabSelected</c:if><c:if test="${panelGroup != 'GO_OUT'}">TabbedPanelsTab</c:if>"><div><a href="<wa:url URI="${site}/content/displayAgenda/group/GO_OUT"/><c:if test="${not empty waParam.caldate}">?caldate=${waParam.caldate}</c:if>"><wa:mls>GO_OUT</wa:mls></a></div></li>
        <!-- <li class="<c:if test="${panelGroup == 'GUIDED_TOUR'}">TabbedPanelsTabSelected</c:if><c:if test="${panelGroup != 'GUIDED_TOUR'}">TabbedPanelsTab</c:if>"><div><a href="<wa:url URI="${site}/content/displayAgenda/group/GUIDED_TOUR"/>"><wa:mls>GUIDED_TOUR</wa:mls></a></div></li>-->
        <!-- <li class="<c:if test="${panelGroup == 'ALL'}">TabbedPanelsTabSelected</c:if><c:if test="${panelGroup != 'ALL'}">TabbedPanelsTab</c:if>"><div><a href="<wa:url URI="${site}/content/displayAgenda/group/ALL"/>"><wa:mls>ALL</wa:mls></a></div></li> -->
      </ul>
      ...
      <c:forEach var="item" items="${list}" varStatus="loop">
          ...
          <div class="txt" id="txt_${item.id}">
            <h3 class="no_margin"><a class="url summary"  href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}"/>" title="${item.mainTitle}"> <p class="description">${item.mainTitle}</p></a></h3>
