		...
		<div class="float_left mtop10">
         ...
         <div id="pratical_information_large">
			...
			<c:if test="${bean.source == 'Atrium'}">
					...
					<a href="<c:choose><c:when test="${culture eq 'BE_en' }">http://www.shopinbrussels.be/EN</c:when><c:when test="${culture eq 'BE_fr' }">http://www.shopinbrussels.be/FR</c:when><c:when test="${culture eq 'BE_nl' }">http://www.shopinbrussels.be/NL</c:when><c:otherwise>http://www.shopinbrussels.be</c:otherwise></c:choose>" target="_blank"><img src="${staticSite}/img/ml/btn_shopinbrussels.jpg" /></a>
			</c:if>  
