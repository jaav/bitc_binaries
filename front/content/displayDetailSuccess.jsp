		...
		<c:if test="${not empty bean.periodFromDate}">
			<h3>
				<c:choose>
					...
					<span class="dtstart value-title" title="<fmt:formatDate pattern="yyyy-MM-dd" value="${bean.periodFromDate}"/>"> 
				...
				-
				 <c:choose>
					<c:when test="${bean.periodToHour eq '00'}">
						<span class="dtend value-title" title="<fmt:formatDate pattern="yyyy-MM-dd" value="${bean.periodToDate}"/>"> 
   ...
   <div class="clr"></div>
    ...
    <c:set var="varTmp" value="\"" />
       ...
       <c:if test="${bean.group ne 'CONTENT' and bean.group ne 'PRESS_REPORT'  and bean.group ne 'OG_TOUR' and bean.group ne 'LATEST_NEWS' and bean.group ne 'PRESTATAIRE'}">
       	 <a href="javascript:<c:choose><c:when test="${section eq 'meetings' || refererMeeting eq '1'}">addToMeetingPlanner</c:when><c:otherwise>addToTripPlanner</c:otherwise></c:choose>('${bean.group}','${bean.id}');"><img src="${staticSite}/img/ml/btn_add_big.gif" alt="<wa:mls>Add</wa:mls>" id="logo_add"/></a>
       	 </c:if>
         ...
         <div id="pratical_information" class="location vcard">
          		...
          		<c:choose>
				    <c:when test="${bean.attachedContentVenue != null}">
				    	<p><strong><wa:mls>Venue :</wa:mls></strong>
				    		<a href="<wa:url URI="${site}/content/displayDetailVenue/group/${bean.attachedContentVenue.group}/id/${bean.attachedContentVenue.id}"/>" ><span >${bean.attachedContentVenue.mainTitle}</span></a> </p>
				    </c:when>
	        ...
	        <c:if test="${contentType ne 'ContentArticle'  && contentGroupType ne 'ContentProduct' and not empty bean.latitude and bean.latitude != 0.0 and not empty bean.longitude and bean.longitude != 0.0}">
			 ...
			 <c:if test="${bean.group == 'GUIDED_TOUR' && bean.source == 'VoirEtDire'}">
			    ...
			    <a href="/bitc/front/tracking/link/desc/voiretdirebruxelles/cid/F0FDB0F455B48382.do" target="_blank"><img src="${staticSite}/img/ml/btn_voiretdire.jpg" alt="voir et dire bruxelles" /></a>
			</c:if>
			 <c:if test="${not empty propTalk}">
					<a href="${propTalk}" target="_blank"><img class="btn_museumtalks" src="${staticSite}/img/ml/btn_museumtalks.jpg" alt="museum talks" /></a>
				</c:if>
			
			<c:if test="${bean.group == 'BnB' && bean.source == 'BednBrussels'}">
					<wa:mls>Pour réservation, cliquez ici</wa:mls><br /><br />
					<a href="${bean.mainUrl}" target="_blank"><img src="${staticSite}/img/ml/btn_bednbrussels.jpg" alt="bed and brussels" /></a>
			</c:if>
