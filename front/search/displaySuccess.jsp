		...
		<ul class="tab_list_complete">
			...
			<c:if test="${result != null}">
				...
				<c:if test="${fn:length(result) > 0}">
					<h1>
						${resultsSize}
						<wa:mls>r&eacute;sultats de recherche</wa:mls>
					</h1>
					<!-- SEARCH RESULTS LIST : start -->
					<c:if test="${group ne 'GUIDED_TOUR'}">

						<c:forEach items="${result}" var="item" varStatus="loop">
							<li><c:if test="${item.contentType ne 'ContentPress'}">
									<c:choose>
										<c:when test="${not empty item.imageUrl}">
											<a href="<wa:url URI='${item.encodedUrl}'/>"> <img
												src="${item.imageUrl}" alt="${item.title}" width="165" />
											</a>
										</c:when>
										<c:otherwise>
											<a href="<wa:url URI='${item.encodedUrl}'/>"> <img
										...
										<c:if test="${item.contentType eq 'ContentEvent'}">
											<c:if test="${not empty item.periodFromDate}">
												...
												<div class="float_left">
													<strong><wa:mls>Date de début :</wa:mls></strong> <span>
														${item.periodFromDate} </span>
												</div>
											</c:if>
											<div class="clr"></div>
											<c:if test="${not empty item.periodToDate}">
												<div class="float_left">
													<strong><wa:mls>Date de fin :</wa:mls></strong> <span>
														${item.periodToDate} </span>
												</div>
											</c:if>
										</c:if>
									</div>
								</c:if> <c:if test="${item.contentType eq 'ContentPress'}">

									<c:choose>
										<c:when test="${not empty item.smallImage}">
											<img src="${item.smallImage}" alt="${item.pressTitle}"
												width="165" />
										</c:when>
										<c:otherwise>
								...
								<div class="btn">
									<c:if
										test="${item.contentType ne 'ContentPress' or not empty item.body}">
										<div class="more_info">
									...
									<c:if
										...
										<a
											href="javascript:callLetter('${wa:numberToChar(loop.count-1)}', 'map')"
										...
										<br />
										<a
											href="javascript:callLetter('${wa:numberToChar(loop.count-1)}', 'sv')"
									...
									<a
										...
										addthis:description="${wa:html2txt(item.title)}"><img
									...
									<c:if test="${group ne 'DEALS'}">
										<a
											href="javascript:addToTripPlanner('${item.group}','${item.id}');"
					...
					<c:if test="${group eq 'GUIDED_TOUR'}">
						<c:if test="${not empty fixedDateTours}">
							<h2>
								<wa:mls>Visits on fixed schedules</wa:mls>
							</h2>
						...
						<c:if test="${not empty onrequestTours}">
							<h2>
								<wa:mls>Visits on request</wa:mls>
							</h2>
