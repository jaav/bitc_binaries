...
<link rel="stylesheet" type="text/css" href="${staticSite}/css/SpryValidationTextField_shop.css"
      media="screen, projection"/>
...
<c:forEach var="item" items="${listEshopProduct}" varStatus="loop">
        ...
        <c:when test="${item.group == 'ES_VOUCHER'}">
            <c:choose>
                <c:when test="${item.listEnableDate==null}">
                    ...
                    <td class="date" valign="top"><span id="tfv_date_product_${item.id}"> <input
                            name="date_product_${item.id}" type="text" id="datePicker_${loop.count}" class="datepicker"
                ...
                <c:when test="${item.listEnableDate != null && fn:length(item.listEnableDate) == 1}">
                    <!-- only one date -->
                    <td class="date"><input name="date_product_${item.id}" type="text" class="datepicker"
                                            value="<fmt:formatDate value="${item.listEnableDate[0]}" pattern="dd/MM/yyyy" />"
                                            readonly="readonly"/></td>
                </c:when>
                <c:when test="${item.listEnableDate != null && fn:length(item.listEnableDate) >= 1}">

                    <td class="date"><span id="tfv_date_product_${item.id}"> <select id="date_product_${item.id}"
                                                                                     name="date_product_${item.id}">
                        <option value="">
    ...
    <c:forEach var="price" items="${item.catalogPrices}" varStatus="loopPrices">
        <c:if test="${price.isActive}">
            ...
            <td class="promoCode"><input type="text" name="promoCode_price_${price.id}" size="12" class="float_right"
                                         value="${price.givenPromo.promoCode}"/></td>
            <td class="space">&nbsp;</td>
            ...
            <td class="quantity">
                <div class="float_left">
                    <a href="javascript:void(0)" class="operator_add" alt="+"><img
                            src="${static}/front/img/ml/addition.jpg" alt="+"/></a><br/> <a href="javascript:void(0)"
...
<div class="box_total">
    <a href="javascript:formRecalculate();" class="btn_update"><wa:mls>Recalculer</wa:mls></a> <a
        href="${context}/${site}/${module}/emptyBasket.do" class="btn_update"><wa:mls>Vider le panier</wa:mls></a>
