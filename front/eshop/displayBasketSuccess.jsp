...
$(document).ready(function(){
  ...
  <div id="content_main">
    ...
    <p><wa:mls>votre panier est vide</wa:mls>
    </c:when>
    <c:otherwise>
	        ...
	        <c:when test="${item.group == 'ES_VOUCHER'}">
		        <c:choose>
		        	<c:when test="${item.listEnableDate==null}">
						...
						$(function() {
			  			...
			  			<td class="date" valign="top">
				  			<span id="tfv_date_product_${item.id}">
				  			<input name="date_product_${item.id}" type="text" id="datePicker_${loop.count}" class="datepicker" value="<fmt:formatDate value="${item.selectedDate}" pattern="dd/MM/yyyy" />" />
		        	...
		        	<c:when test="${item.listEnableDate != null && fn:length(item.listEnableDate) == 1}">
		        		<!-- only one date -->
		        		<td class="date">
		        			<input name="date_product_${item.id}" type="text" class="datepicker" value="<fmt:formatDate value="${item.listEnableDate[0]}" pattern="dd/MM/yyyy" />" readonly="readonly"/>
	          			</td>
		        	</c:when>
		        	<c:when test="${item.listEnableDate != null && fn:length(item.listEnableDate) >= 1}">
						...
						<td class="date">
						<span id="tfv_date_product_${item.id}">
							<select id="date_product_${item.id}" name="date_product_${item.id}">
				            	<option value=""><wa:mls>Choisissez</wa:mls></option>
				           		<c:forEach var="date" items="${item.listEnableDate}" varStatus="loopDates">
									<option value="<fmt:formatDate value="${date}" pattern="dd/MM/yyyy" />"><fmt:formatDate value="${date}" pattern="dd/MM/yyyy" /></option>
			        			</c:forEach>
				          	</select>
	        ...
	        <c:forEach var="price" items="${item.catalogPrices}" varStatus="loopPrices">
	        	<c:if test="${price.isActive}">
			        ...
			        <td class="quantity"><div class="float_left"> 
			          ...
			          <input name="quantity_product_${item.id}_price_${price.id}" type="text" id="quantity_product_${item.id}_price_${price.id}" value="${price.quantity}" size="4" class="float_right" /> 
			          </td>
			        <td class="space">&nbsp;</td>
    ...
    <div class="box_total">    
	 	<a href="javascript:formRecalculate();" class="btn_update"><wa:mls>Recalculer</wa:mls></a>
	 	<a href="${context}/${site}/${module}/emptyBasket.do" class="btn_update"><wa:mls>Vider le panier</wa:mls></a>    
	 	<div class="group">
