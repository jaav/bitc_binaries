	...
	<div id="searchBox" style="width:300px">
		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}">
				<div>
					<label for="s_contentArticleId"><wa:mls>ProductId</wa:mls></label>
					<input name="s_contentArticleId" type="text" <c:choose><c:when test="${waParam.s_contentArticleId!=null}">value="${waParam.s_contentArticleId}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
				<div>
				...
				<div>
					<label for="s_body"><wa:mls>Description</wa:mls></label>
				...
				<!-- 
				<div>
					<label for="s_publishContent_contentArticleId"><wa:mls>publishContent</wa:mls></label>
					<input name="s_publishContent_contentArticleId" type="text" <c:choose><c:when test="${waParam.s_publishContent_contentArticleId!=null}">value="${waParam.s_publishContent_contentArticleId}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
				-->
				...
				<c:if test="${fn:length(listProperties) > 0}">
					...
					<c:forEach var="property" items="${listProperties}">
						<div>
							<label for="s_property_link"><wa:mls>${property.name}</wa:mls></label>		
		...
		<table cellspacing=0 cellpadding=0>
			<tr class="header">
				...
				<td class="center checkbox">
					...
					<td class="center contentArticleId">
						<wa:link URI="${URI}" allParams="true">
							...
							<wa:param name="orderBy" value="contentArticleId" />
						</wa:link>			
					</td>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					>&nbsp;</td>
					<td class="center checkbox"><input type="checkbox" value="${item.contentArticleId}"></td>
					<td class="center contentArticleId"><a href="${context}/${site}/product/editItem/id/${item.contentArticleId}.do" title="<wa:mls>Edit id</wa:mls>">${item.contentArticleId}</a></td>
					...
					<td class="center publishContent">${item.publishContent.contentArticleId}&nbsp;</td>
					<td class="center actions">
						<a href="javascript:deleteInLine('${item.contentArticleId}');" title="<wa:mls>Delete</wa:mls>"><img src="${static}/${site}/img/ico_delete.png"  alt="<wa:mls>Delete</wa:mls>"/></a>
						<c:if test="${item.status != 1}">
							<a href="javascript:publishInLine('${item.contentArticleId}');" title="<wa:mls>Publish</wa:mls>"><img src="${static}/${site}/img/ico_publish.png" alt="<wa:mls>Publish</wa:mls>"/></a>
						</c:if>
						<c:if test="${item.group == 'NEWS_BEL'}">
							<a href="${context}/${site}/${module}/duplicate/id/${item.contentArticleId}.do" title="<wa:mls>Duplicate</wa:mls>"><img src="${static}/${site}/img/ico_duplicate.png" alt="<wa:mls>Duplicate</wa:mls>"/></a>
						</c:if>
...
<div id="fieldList">
		<div>
			<input name="contentArticleId" id="c_contentArticleId" type="checkbox" value="1" checked="checked"/>
			<label><wa:mls>ProductId</wa:mls></label>
		</div>
		<div>
