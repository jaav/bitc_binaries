...
<h2><wa:mls>acxUserprofile</wa:mls>&nbsp;-&nbsp;
		...
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm">
			<fieldset>
				...
				<c:if test="${bean!=null && bean.id>0}">
					...
					<div class="bgWhite"> 
						<label for="f_adminuserid"><wa:mls>adminuserid</wa:mls></label>
						<textarea name="f_adminuserid"  id="f_adminuserid" class="textfield" >${bean.adminuserid}</textarea>
						<span><a href="<wa:url URI="${site}/acxAdminusers/acxAdminUserList/openerField/f_adminuserid" />" target="users">Users</a></span>
					</div>
					<div class="bgGrey">
						<label for="f_profile"><wa:mls>profile</wa:mls></label>
						<!-- $attribute.findTaggedValue("activeidea.attribute.alias") -->
						<wa:include URI="${site}/acxProfile/dropList">
							<wa:param name="def" value="${bean.profile}"/>
							<wa:param name="selectName" value="f_profile"/>
						</wa:include>
					</div>
				</fieldset>
			<div id="errorMessage">
