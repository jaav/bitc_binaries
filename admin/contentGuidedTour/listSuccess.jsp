	...
	<div id="searchBox" style="width:300px">
		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}">
				...
				<div>
					<label for="s_numParticipation"><wa:mls>Num Participation</wa:mls></label>
					<input name="s_numParticipation" type="text" <c:choose><c:when test="${waParam.s_numParticipation!=null}">value="${waParam.s_numParticipation}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
				<div>
					<label for="s_highlight"><wa:mls>Highlight</wa:mls></label>
					<input type="radio"<c:if test="${waParam.s_highlight=='0'}"> checked="checked"</c:if> value="0" name="s_highlight" style="margin-left:50px;"/><wa:mls>No</wa:mls>
					<input type="radio"<c:if test="${waParam.s_highlight=='1'}"> checked="checked"</c:if> value="1" name="s_highlight"/><wa:mls>Yes</wa:mls>
				</div>
...
<!--floatLeft:end-->
<h2><wa:mls>contentGuidedTour${waParam.group}</wa:mls></h2>
<div id="list">
	<div id="mask">
		<table cellspacing=0 cellpadding=0>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					<td class="center id"><a href="${context}/${site}/contentGuidedTour/editItem/id/${item.id}.do" title="<wa:mls>Edit id</wa:mls>">${item.id}</a></td>
