	...
	<div class="content" id="actions">
		...
		<span id="t_category_1" > 
			<wa:include URI="${site}/dropdown/dropList">
				...
				<wa:param name="name" value="t_category" />
				<wa:param name="selectedValues" value="" />
			</wa:include>
		</span>
		<a href="javascript:void(0);" id="changeCategory_btn" ><wa:mls>change category</wa:mls></a>
...
<!--floatLeft:end-->
<h2><wa:mls>contentVenue${waParam.group}</wa:mls></h2>
<div id="list">
	<div id="mask">
		<table cellspacing=0 cellpadding=0>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					<td class="center id"><a href="${context}/${site}/contentVenue/editItem/id/${item.id}.do" title="<wa:mls>Edit id</wa:mls>">${item.id}</a></td>
					...
					<td class="center rooms">
						<a href="${context}/${site}/contentRoom/list.do?s_contentVenue_id=${item.id}&s_contentVenue_name=${item.name}"><wa:mls>rooms</wa:mls></a><br />
						<a href="${context}/${site}/contentRoom/newItem.do?s_contentVenue_id=${item.id}&s_contentVenue_name=${item.name}"><wa:mls>rooms-add</wa:mls></a> 
					</td>
				</tr>
			</c:forEach>
		</table>
...
var cc_btn = $('changeCategory_btn');
if(cc_btn!=null)
	cc_btn.addClass('atLeastOne');
cc_btn.addEvent('click',function(){
	...
	var cfrm=confirm('Do you want to change the category of this elements : '+id);
	if (cfrm==true){
		var t_category = $$('select[name="t_category"]')[0].value;
		cc_btn.href=path+'/changeCategoryItems/items/'+id+'/categ/'+t_category+'.do';
	}
});
