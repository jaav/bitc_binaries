	...
	<div id="searchBox">
		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}" >
				...
				<div>
					<label for="s_cssFile"><wa:mls>cssFile</wa:mls></label>
					<input name="s_cssFile" type="text" <c:choose><c:when test="${waParam.s_cssFile!=null}">value="${waParam.s_cssFile}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
				<div>
					<label for="s_numItems"><wa:mls>numItems</wa:mls></label>
					<input name="s_numItems" type="text" <c:choose><c:when test="${waParam.s_numItems!=null}">value="${waParam.s_numItems}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
				<div class="">
...
<!--floatLeft:end-->
<h2><wa:mls>Calendar List</wa:mls></h2>
<div id="list">
	<div id="mask">
		<table cellspacing=0 cellpadding=0>
			<tr class="header">
				...
				<td class="left height">
						...
						<td class="left cssFile">
							<wa:link URI="${URI}" allParams="true">
								<wa:mls>cssFile</wa:mls>
								<wa:exceptParam name="page"/>
								<wa:param name="page" value="1" />
								<wa:exceptParam name="orderDir"/>
								<wa:param name="orderDir" value="${orderDir}" />
								<wa:exceptParam name="orderBy"/>
								<wa:param name="orderBy" value="cssFile" />
							</wa:link>			
						</td>
					<td class="left numItems">
							<wa:link URI="${URI}" allParams="true">
								<wa:mls>numItems</wa:mls>
								<wa:exceptParam name="page"/>
								<wa:param name="page" value="1" />
								<wa:exceptParam name="orderDir"/>
								<wa:param name="orderDir" value="${orderDir}" />
								<wa:exceptParam name="orderBy"/>
								<wa:param name="orderBy" value="numItems" />
							</wa:link>			
						</td>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					<td class="center id"><a href="${context}/${site}/calendarList/editItem/id/${item.widgetId}.do" title="<wa:mls>Edit id</wa:mls>">${item.widgetId}</a></td>
					<td class="center partnerId">
					...
					<td class="left cssFile"><a href="${urlFile}${item.cssFile}" target="_blanc" type="text/css"><wa:mls>${item.cssFile}</wa:mls></a></td>
					<td class="left numItems">${item.numItems}</td>
					<td class="left valid">${item.valid}</td>
					<td class="left createdOn"><fmt:formatDate pattern="dd/MM/yyyy hh:mm" value="${item.createdOn}"/>&nbsp;</td>
					<td class="left modifiedOn"><fmt:formatDate pattern="dd/MM/yyyy hh:mm" value="${item.modifiedOn}"/>&nbsp;</td>
...
<div id="fieldList">
	...
	<div>
		<input name="cssFile" id="c_cssFile" type="checkbox" value="1" checked="checked"/>
		<label><wa:mls>cssFile</wa:mls></label>
	</div>
	...
	<div>
		<input name="numItems" id="c_numItems" type="checkbox" value="1" checked="checked"/>
		<label><wa:mls>numItems</wa:mls></label>
	</div>
