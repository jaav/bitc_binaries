...
<input name="widgetId" type="hidden"/>
			...
			<div>
			<label for="f_calendarPartner_id" id="v_calendarPartner"><wa:mls>calendarPartner</wa:mls></label>
					<c:choose><c:when test="${bean.calendarPartner.partnerId > 0}"><c:set var="partnerId" value="${bean.calendarPartner.partnerId}" scope="page"/></c:when><c:otherwise><c:set var="partnerId" value="${waParam.f_calendarPartner_partnerId}" scope="page"/></c:otherwise></c:choose>
					<wa:include URI="${site}/dropdown/dropList">
							...
							<wa:param name="name" value="f_partnerId" />
		...
		<div class="bgGrey" id="v_numItems">
					<div class="">
					<label for="f_numItems"><wa:mls>numItems</wa:mls></label>
					<input name="f_numItems" type="text" value="<c:if test="${bean.widgetId == null}">10</c:if><c:if test="${bean.widgetId != null}">${bean.numItems}</c:if>"/><br/>
					<div class="textfieldRequiredMsg"><img src="${context}/static/admin/img/erreur.gif"/><wa:mls>Ce champ doit contenir uniquement des chiffres</wa:mls></div>
					</div>
					<script type="text/javascript">
						var v_numItems = new Spry.Widget.ValidationTextField("v_numItems", "integer", {validateOn:["blur"], isRequired:false});
					</script> 
				
		</div>
		<div class="bgWhite" id="v_cssFile">
		<div class="">
				<label for="f_cssFile"><wa:mls>cssFile</wa:mls></label>
				<textarea name="f_cssFile" class="textfield" ><c:choose><c:when test="${not empty bean.cssFile}">${bean.cssFile}</c:when><c:otherwise>http://visitbrussels.be/bitc/static/iframe/css/screen.css</c:otherwise></c:choose></textarea><br/>
				<div class="textfieldRequiredMsg"><img src="${context}/static/admin/img/erreur.gif"/><wa:mls>Ce champ est obligatoire</wa:mls></div>
				</div>
		...
		<c:if test="${bean.widgetId != null && bean.widgetId != 0}">
			<div class="bgWhite">
					<div class="">
						...
						&lt;iframe height="${bean.height}" width="${bean.width}" src="${domain}/bitc/iframe/calendar/list.do?code=${bean.calendarPartner.cryptedId}-${bean.widgetId}&lg=fr&date=&categ="&gt;&lt;/iframe&gt;
					</div>
						...
						<c:forEach var="property" items="${listProperties}">
							<div>
								<wa:include URI="admin/dropdown/dropListProperty">
									...
									<wa:param name="contentType" value="CalendarList"/>
