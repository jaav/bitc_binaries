		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}"> 
				...
				<div>
					...
					<input name="s_ressource" type="text" <c:choose><c:when test="${waParam.s_ressource!=null}">value="${waParam.s_ressource}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
...
<!--floatLeft:end-->
<h2><wa:mls>acxRessource</wa:mls></h2>
<div id="list">
	<div id="mask">
		<table cellspacing=0 cellpadding=0>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					<td class="center id"><a href="${context}/${site}/acxRessource/editItem/id/${item.id}.do" title="<wa:mls>Edit id</wa:mls>">${item.id}</a></td>
