		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}"> 
				...
				<div>
					...
					<input name="s_right" type="text" <c:choose><c:when test="${waParam.s_right!=null}">value="${waParam.s_right}"</c:when><c:otherwise>value=""</c:otherwise></c:choose>/>
				</div>
		...
		<table cellspacing=0 cellpadding=0>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
					...
					<td class="center id"><a href="${context}/${site}/acxRight/editItem/id/${item.id}.do" title="<wa:mls>Edit id</wa:mls>">${item.id}</a></td>
