		...
		<div class="content">
			<form name="searchForm" id="searchForm" action="${context}/${site}/${module}/${action}.do${filterString}">
				...
				<div>
					...
					<c:if test="${property.type!='FREE' && property.type!='DB'}">
					<wa:include URI="admin/dropdown/dropList">
						...
						<wa:param name="type" value="${property.type}" />
					</wa:include>
					</c:if>
					<c:if test="${property.type=='DB'}">
						<wa:include URI="admin/dropdown/dropList">
							<wa:param name="class" value="${property.className}" />
							<wa:param name="manager" value="${property.manager}" />
							<wa:param name="identity" value="${property.identity}" />
							<wa:param name="display" value="${property.identity}" />
							<wa:param name="display2" value="${property.display}" />
							<wa:param name="method" value="${property.method}" />
							<wa:param name="propertyName" value="${property.name}" />
							<wa:param name="name" value="s_property_link_${fn:toLowerCase(property.name)}" />
							<wa:param name="selectedValues" value="${waParam[property.propertyLinkName]}" />
							<wa:param name="type" value="${property.type}" />
...
<!--floatLeft:end-->
<h2><wa:mls>ContentArticle${waParam.group}</wa:mls></h2>
<div id="list">
	<div id="mask">
		<table cellspacing=0 cellpadding=0>
			...
			<c:forEach var="item" items="${list}" varStatus="loop">
				<tr class="line${((loop.count+1)%2)+1}">
						...
						<c:if test="${item.status == 4}">class="pinkStatus"</c:if>
						<c:if test="${item.status == 5}">class="yellowStatus"</c:if>
					>&nbsp;</td>
					<td class="center checkbox"><input type="checkbox" value="${item.id}"></td>
					<td class="center id"><a href="${context}/${site}/contentArticle/editItem/id/${item.id}.do" title="<wa:mls>Edit id</wa:mls>">${item.id}</a></td>
					...
					<td class="center preview">
						<c:if test="${item.group ne 'ES_CATEGORY' && item.group ne 'INTRO_LIST' && item.group ne 'INTRO_FORM' && item.group ne 'CONFIRMATION' && item.group ne 'RIGHT' && item.group ne 'Splash' && item.group ne 'NEWSLETTER'}">
							<a href="${context}/front/content/displayDetail/group/${item.group}/id/${item.id}.do" target="site"><wa:mls>Preview</wa:mls></a>
						</c:if>
						<c:if test="${item.group eq 'ES_CATEGORY'}">
							<a href="${context}/front/eshop/displayEsProduct/id/${item.id}.do" target="site"><wa:mls>Preview</wa:mls></a>
						</c:if> 
					</td>
					<td class="center actions">
