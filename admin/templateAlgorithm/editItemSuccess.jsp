...
<h2><wa:mls>Algorithm</wa:mls>&nbsp;-&nbsp;
	<c:if test="${bean==null || bean.algorithmId<=0}"><wa:mls>(Add)</wa:mls></c:if>
	<c:if test="${bean!=null && bean.algorithmId>0}"><wa:mls>(Edit)</wa:mls></c:if>
</h2>
<div id="list">
	<div id="mask">
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm">
			<fieldset>
				<legend>&nbsp;</legend>
				<c:if test="${bean!=null && bean.algorithmId>0}">
				<div class="bgGrey">
					<label for="f_id"><wa:mls>AlgorithmId</wa:mls></label>
					<span>${bean.algorithmId}</span>
					<input name=id type="hidden" value="${bean.algorithmId}"/>
				</div>
				</c:if>
				<div class="bgWhite">
				...
				<div class="bgWhite">
					<label for="f_params"><wa:mls>Params</wa:mls></label>
					<textarea name="f_params" class="textfield" >${bean.params}</textarea>
				</div>
			</fieldset>
			<div id="errorMessage">
