...
<h2><wa:mls>QmQuestionnaire</wa:mls>&nbsp;-&nbsp;
	<c:if test="${bean==null || bean.qmQuestionnaireId<=0}"><wa:mls>(Add)</wa:mls></c:if>
	<c:if test="${bean!=null && bean.qmQuestionnaireId>0}"><wa:mls>(Edit)</wa:mls></c:if>
</h2>
<div id="list">
	<div id="mask">
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm">
			<fieldset>
				<legend>&nbsp;</legend>
				<c:if test="${bean!=null && bean.qmQuestionnaireId>0}">
				<div class="bgGrey">
					<label for="f_id"><wa:mls>QmQuestionnaireId</wa:mls></label>
					<span>${bean.qmQuestionnaireId}</span>
					<input name=id type="hidden" value="${bean.qmQuestionnaireId}"/>
				</div>
				</c:if>
				<div class="bgWhite">
				...
				<div>
					<label for="f_qmGroup_qmGroupId"><wa:mls>QmGroup</wa:mls></label>
					<wa:include URI="${site}/dropdown/dropList">
						...
						<wa:param name="name" value="f_qmGroup_qmGroupId" />
						<wa:param name="choiceMsg" value="Choose the Group" />
						<wa:param name="selectedValues" value="${bean.qmGroup.qmGroupId}" />
					</wa:include>
				</div>
			<c:if test="${bean!=null && bean.qmQuestionnaireId>0}">
				<div class="bgGrey">
