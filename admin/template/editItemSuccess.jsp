...
<h2><wa:mls>Template</wa:mls>&nbsp;-&nbsp;
	<c:if test="${bean==null || bean.templateId<=0}"><wa:mls>(Add)</wa:mls></c:if>
	<c:if test="${bean!=null && bean.templateId>0}"><wa:mls>(Edit)</wa:mls></c:if>
</h2>
<div id="list">
	<div id="mask">
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm">
			<fieldset>
				<legend>&nbsp;</legend>
				<c:if test="${bean!=null && bean.templateId>0}">
				<div class="bgGrey">
					<label for="f_id"><wa:mls>TemplateId</wa:mls></label>
					<span>${bean.templateId}</span>
					<input name=id type="hidden" value="${bean.templateId}"/>
				</div>
				</c:if>
				<div class="bgWhite">
