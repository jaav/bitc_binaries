		...
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm" name="detailForm">
			<fieldset>
				...
				<c:if test="${bean!=null && bean.id>0 && duplicate eq null}">
					...
					<div class="bgWhite" id="v_group">
						...
						<c:if test="${bean.group==null || bean.group ==''}">
							<wa:include URI="${site}/dropdown/dropList">
								...
								<wa:param name="method" value="listByTypeContentPlace" />
					...
					<fieldset>
						<legend><wa:mls>Place Infos</wa:mls></legend>
						<div class="bgGrey" id="v_adresseFr">
						...
						<div class="bgGrey" id="v_latitude">
					...
					<c:if test="${bean!=null && bean.source!=null && bean.contentPlaceClts[clt].sourceTitle!=null}">
					<div class="bgGrey">
						<label for="f_${clt}_sourceTitle"><wa:mls>title </wa:mls> (source)</label>
						<span>${bean.contentPlaceClts[clt].sourceTitle}</span>
					</div>
					</c:if>		
					<div class="bgGrey" style="height:50px;" id="v_${clt}_title">
						<label for="f_${clt}_title"  style="float:left;"><wa:mls>title</wa:mls></label>
						<textarea name="f_${clt}_title" id="f_${clt}_title" style="float:left;" class="textfield textForAll" >${bean.contentPlaceClts[clt].title}</textarea>
						<div class="textareaRequiredMsg"><img src="${context}/static/admin/img/erreur.gif"/><wa:mls>Ce champ est obligatoire</wa:mls></div>
	            		<script type="text/javascript">
					...
					<c:if test="${bean!=null && bean.source!=null && bean.contentPlaceClts[clt].sourceAbstract!=null}">
					<div class="bgWhite">
						<label for="f_${clt}_sourceAbstract_Not" style="float:left;"><wa:mls>abstract_ (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceAbstractNot" id="f_${clt}_sourceAbstractNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentPlaceClts[clt].sourceAbstract}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgWhite abstract">
						...
						<textarea name="f_${clt}_abstract_" id="f_${clt}_abstract_" class="textarea2" style="float:left; margin-left:3px;">${bean.contentPlaceClts[clt].abstract_}</textarea>
						</div>
						<div class="clr"></div>
					</div>
	
					<c:if test="${bean!=null && bean.source!=null && bean.contentPlaceClts[clt].sourceBody!=null}">
					<div class="bgGrey">
						<label for="f_${clt}_sourceBodyNot" style="float:left;"><wa:mls>body (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceBodyNot" id="f_${clt}_sourceBodyNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentPlaceClts[clt].sourceBody}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgGrey body">
						...
						<div  style="float:left; margin-left:3px; width:95%">
							<textarea name="f_${clt}_body" id="f_${clt}_body"  class="textarea2" >${bean.contentPlaceClts[clt].body}</textarea>
						</div>
						<div class="clr"></div>	
						
					</div>
					<c:if test="${bean!=null && bean.source!=null && bean.contentPlaceClts[clt].sourceInfo!=null}">
					<div class="bgWhite">
						<label for="f_${clt}_sourceInfoNot" style="float:left;"><wa:mls>info (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceInfoNot" id="f_${clt}_sourceInfoNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentPlaceClts[clt].sourceInfo}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgWhite info">
						...
						<div  style="float:left; margin-left:3px; width:95%">
							<textarea name="f_${clt}_info" id="f_${clt}_info"  class="textarea2" >${bean.contentPlaceClts[clt].info}</textarea>
						</div>
						<div class="clr"></div>	
					...
					<div class="bgGrey" id="v_${clt}_bigImage">
						...
						<textarea name="f_${clt}_bigImage" id="f_${clt}_bigImage" class="textfield bigImageForAll" style="float:left; margin-left:3px">${bean.contentPlaceClts[clt].bigImage}</textarea>
					...
					<div class="bgWhite" id="v_${clt}_smallImage">
						...
						<textarea name="f_${clt}_smallImage" id="f_${clt}_smallImage" class="textfield smallImageForAll" style="float:left; margin-left:3px">${bean.contentPlaceClts[clt].smallImage}</textarea>
					...
					<div class="bgGrey" id="v_${clt}_url">
						...
						<textarea name="f_${clt}_url"  id="f_${clt}_url" class="textfield urlForAll"  style="float:left; margin-left:3px">${bean.contentPlaceClts[clt].url}</textarea>
					...
					<div class="bgWhite" id="v_${clt}_urlLabel">
						...
						<textarea name="f_${clt}_urlLabel" class="textfield" >${bean.contentPlaceClts[clt].urlLabel}</textarea>
						<script type="text/javascript">
					...
					<div class="bgGrey">
						...
						<span><c:if test="${bean.contentPlaceClts[clt].openCount==null}">0</c:if>${bean.contentPlaceClts[clt].openCount}</span>
					</div>
					<div class="bgWhite">
						<label for="f_${clt}_commentCount"><wa:mls>commentCount</wa:mls></label>
						<span><c:if test="${bean.contentPlaceClts[clt].commentCount==null}">0</c:if>${bean.contentPlaceClts[clt].commentCount}</span>
					</div>
					</c:if>
						</div>
						</c:forEach>
				...
				<c:choose>
						...
						<c:forEach var="property" items="${listProperties}">
							<div>
								<wa:include URI="admin/dropdown/dropListProperty">
									...
									<wa:param name="contentType" value="ContentPlace"/>
					...
					<div class="clr"></div>
				 	 ...
				 	 <a href="<wa:url URI="${site}/googleStreetView/display/ContentType/ContentPlace/id/${bean.id}"/>" target="admin"><wa:mls>Google Street View </wa:mls></a>
...
$('f_group').addEvent('change',function(){ 
	var myRequest = new Request({
		url : '${context}/${site}/ajax/properties.do?type=article&realType=ContentPlace',
		method :'get',
