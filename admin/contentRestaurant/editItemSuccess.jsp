...
<h2><wa:mls>contentRestaurant</wa:mls>&nbsp;-&nbsp;
		...
		<form action="${context}/${site}/${module}/saveItem.do" method="post" id="detailForm" name="detailForm">
			<fieldset>
				...
				<c:if test="${bean!=null && bean.id>0 && duplicate eq null}">
					...
					<div class="bgWhite" id="v_group">
						...
						<c:if test="${bean.group==null || bean.group ==''}">
							<wa:include URI="${site}/dropdown/dropList">
								...
								<wa:param name="method" value="listByTypeContentRestaurant" />
					...
					<fieldset>
						<legend><wa:mls>Restaurant Infos</wa:mls></legend>
						...
						<div class="bgWhite" id="v_ranking">
							<label for="f_ranking"><wa:mls>ranking</wa:mls></label>
							<textarea name="f_ranking" class="textfield" >${bean.ranking}</textarea>
							<script type="text/javascript">
								var v_ranking = new Spry.Widget.ValidationTextarea("v_ranking", {isRequired:false,maxChars:10});
							</script> 
						</div>
						<div class="bgWhite" id="v_capacity">
							<label for="f_capacity"><wa:mls>capacity</wa:mls></label>
							<textarea name="f_capacity" class="textfield" >${bean.capacity}</textarea>
							<script type="text/javascript">
								var v_capacity = new Spry.Widget.ValidationTextarea("v_capacity", {isRequired:false,maxChars:10});
							</script> 
						</div>
						<div class="bgGrey">
							<label for="f_valid"><wa:mls>valid</wa:mls></label>
							<input type="radio"<c:if test="${bean.valid ==  null || bean.valid==false}"> checked="checked"</c:if> value="0" name="f_valid"/><wa:mls>no</wa:mls>
							<input type="radio"<c:if test="${bean.valid==true}"> checked="checked"</c:if> value="1" name="f_valid"/><wa:mls>yes</wa:mls>
						</div>
						<div class="bgGrey">
							<label for="f_gourmet"><wa:mls>gourmet</wa:mls></label>
							<input type="radio"<c:if test="${bean.gourmet ==  null || bean.gourmet==false}"> checked="checked"</c:if> value="0" name="f_gourmet"/><wa:mls>no</wa:mls>
							<input type="radio"<c:if test="${bean.gourmet==true}"> checked="checked"</c:if> value="1" name="f_gourmet"/><wa:mls>yes</wa:mls>
						</div>
						<div class="bgGrey">
							<label for="f_breakfast"><wa:mls>breakfast</wa:mls></label>
							<input type="radio"<c:if test="${bean.breakfast ==  null || bean.breakfast==false}"> checked="checked"</c:if> value="0" name="f_breakfast"/><wa:mls>no</wa:mls>
							<input type="radio"<c:if test="${bean.breakfast==true}"> checked="checked"</c:if> value="1" name="f_breakfast"/><wa:mls>yes</wa:mls>
						</div>
						<div class="bgGrey">
							<label for="f_night"><wa:mls>night</wa:mls></label>
							<input type="radio"<c:if test="${bean.night ==  null || bean.night==false}"> checked="checked"</c:if> value="0" name="f_night"/><wa:mls>no</wa:mls>
							<input type="radio"<c:if test="${bean.night==true}"> checked="checked"</c:if> value="1" name="f_night"/><wa:mls>yes</wa:mls>
						</div>
					</fieldset>
					<div class="clr"></div>
					...
					<c:if test="${bean!=null && bean.source!=null && bean.contentRestaurantClts[clt].sourceTitle!=null}">
					<div class="bgGrey">
						<label for="f_${clt}_sourceTitle"><wa:mls>title </wa:mls> (source)</label>
						<span>${bean.contentRestaurantClts[clt].sourceTitle}</span>
					</div>
					</c:if>		
					<div class="bgGrey" style="height:50px;" id="v_${clt}_title">
						<label for="f_${clt}_title" style="float:left;"><wa:mls>title</wa:mls></label>
						<textarea name="f_${clt}_title" style="float:left;" id="f_${clt}_title" class="textfield textForAll" >${bean.contentRestaurantClts[clt].title}</textarea>
						<div class="textareaRequiredMsg"><img src="${context}/static/admin/img/erreur.gif"/><wa:mls>Ce champ est obligatoire</wa:mls></div>
	            		<script type="text/javascript">
					...
					<c:if test="${bean!=null && bean.source!=null && bean.contentRestaurantClts[clt].sourceAbstract!=null}">
					<div class="bgWhite">
						<label for="f_${clt}_sourceAbstract_Not" style="float:left;"><wa:mls>abstract_ (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceAbstractNot" id="f_${clt}_sourceAbstractNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentRestaurantClts[clt].sourceAbstract}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgWhite abstract">
						...
						<textarea name="f_${clt}_abstract_" id="f_${clt}_abstract_" class="textarea2" style="float:left; margin-left:3px;">${bean.contentRestaurantClts[clt].abstract_}</textarea>
						</div>
						<div class="clr"></div>
					</div>
	
					<c:if test="${bean!=null && bean.source!=null && bean.contentRestaurantClts[clt].sourceBody!=null}">
					<div class="bgGrey">
						<label for="f_${clt}_sourceBodyNot" style="float:left;"><wa:mls>body (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceBodyNot" id="f_${clt}_sourceBodyNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentRestaurantClts[clt].sourceBody}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgGrey body">
						...
						<div  style="float:left; margin-left:3px; width:95%">
							<textarea name="f_${clt}_body" id="f_${clt}_body"  class="textarea2" >${bean.contentRestaurantClts[clt].body}</textarea>
						</div>
						<div class="clr"></div>	
						
					</div>
					<c:if test="${bean!=null && bean.source!=null && bean.contentRestaurantClts[clt].sourceInfo!=null}">
					<div class="bgWhite">
						<label for="f_${clt}_sourceInfoNot" style="float:left;"><wa:mls>info (source) - Not editable</wa:mls></label>
						<textarea name="f_${clt}_sourceInfoNot" id="f_${clt}_sourceInfoNot" class="textarea2" style="float:left; margin-left:3px;">${bean.contentRestaurantClts[clt].sourceInfo}</textarea>
						<div class="clr"></div>	
					</div>
					</c:if>
					...
					<div class="bgWhite info">
						...
						<div  style="float:left; margin-left:3px; width:95%">
							<textarea name="f_${clt}_info" id="f_${clt}_info"  class="textarea2" >${bean.contentRestaurantClts[clt].info}</textarea>
						</div>
						<div class="clr"></div>	
					...
					<div class="bgGrey" id="v_${clt}_bigImage">
						...
						<textarea name="f_${clt}_bigImage" id="f_${clt}_bigImage" class="textfield bigImageForAll" style="float:left; margin-left:3px">${bean.contentRestaurantClts[clt].bigImage}</textarea>
					...
					<div class="bgWhite" id="v_${clt}_smallImage">
						...
						<textarea name="f_${clt}_smallImage" id="f_${clt}_smallImage" class="textfield smallImageForAll" style="float:left; margin-left:3px">${bean.contentRestaurantClts[clt].smallImage}</textarea>
					...
					<div class="bgGrey" id="v_${clt}_url">
						...
						<textarea name="f_${clt}_url"  id="f_${clt}_url" class="textfield urlForAll"  style="float:left; margin-left:3px">${bean.contentRestaurantClts[clt].url}</textarea>
					...
					<div class="bgWhite" id="v_${clt}_urlLabel">
						...
						<textarea name="f_${clt}_urlLabel" class="textfield" >${bean.contentRestaurantClts[clt].urlLabel}</textarea>
						<script type="text/javascript">
					...
					<div class="bgGrey">
						...
						<span><c:if test="${bean.contentRestaurantClts[clt].openCount==null}">0</c:if>${bean.contentRestaurantClts[clt].openCount}</span>
					</div>
					<div class="bgWhite">
						<label for="f_${clt}_commentCount"><wa:mls>commentCount</wa:mls></label>
						<span><c:if test="${bean.contentRestaurantClts[clt].commentCount==null}">0</c:if>${bean.contentRestaurantClts[clt].commentCount}</span>
					</div>
					...
					<fieldset>
						...
						<textarea name="f_${clt}_price" class="textfield" >${bean.contentRestaurantClts[clt].price}</textarea>
						<script type="text/javascript">
						...
						<div class="bgWhite" id="v_${clt}_descriptionPrice">
							...
							<textarea name="f_${clt}_descriptionPrice" class="textfield" >${bean.contentRestaurantClts[clt].descriptionPrice}</textarea>
							<script type="text/javascript">
						...
						<c:forEach var="property" items="${listProperties}">
							...
							<div>
								<wa:include URI="admin/dropdown/dropListProperty">
									...
									<wa:param name="contentType" value="ContentRestaurant"/>
...
<div class="clr"></div>
		           <fieldset>
				 	 ...
				 	 <a href="<wa:url URI="${site}/googleStreetView/display/ContentType/ContentRestaurant/id/${bean.id}"/>" target="admin"><wa:mls>Google Street View </wa:mls></a>
...
$('f_group').addEvent('change',function(){ 
	var myRequest = new Request({
		url : '${context}/${site}/ajax/properties.do?type=article&realType=ContentRestaurant',
		method :'get',
