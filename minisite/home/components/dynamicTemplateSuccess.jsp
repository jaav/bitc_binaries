...
<c:if test="${module eq 'home'}">
<div id="title" class="title">
<h1 class="title_h1">${minisiteClt.title}</h1></div>
</c:if>	
				 	...
				 	<div>
				 		<c:choose>
				 			<c:when test="${item.homeDisplay eq 'free'}">
				 				<div class="first_highlight_home_slide">
				 					${item.mainBody}
				 				</div>
				 			</c:when>
				 			<c:otherwise>
				      	...
				      	<div class="first_highlight_home_slide">
				             <h2>
				             	<c:choose>
		        					<c:when test="${item.group == 'HOTEL'}">
					   					<a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" ><span class="cutH2Title">${item.mainTitle}</span></a>
				    				</c:when>
      			    				<c:otherwise>
	      			    				<a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" ><span class="cutH2Title">${item.mainTitle}</span></a>
      			    				</c:otherwise>
      							</c:choose>
      						</h2>
      						<h3>
				           	...
				           	<c:choose>
          						<c:when test="${not empty item.mainSmallImage}">
	        						<c:choose>
		        						<c:when test="${item.group == 'HOTEL'}">
					    					<a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				        						<img src="${item.mainSmallImage}" alt="${item.mainTitle}" width="410"/>
				        					</a>
				    					</c:when>
      			    					<c:otherwise>
          						...
          						<c:otherwise>
          							<c:choose>
		        						<c:when test="${item.group == 'HOTEL'}">
					    					<a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				        						<img src="${context}/static/front/img/ml/default_content.jpg" alt="${item.mainTitle}" width="410"/>
				        					</a>
				    					</c:when>
      			    					<c:otherwise>
	      			    					<a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				            ...
				            <div class="btn">
								<div class="more_info">
									<c:choose>
				        				<c:when test="${item.group == 'HOTEL'}">
							   				<a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" class="btn_more_info" title="${item.mainTitle}"><wa:mls>More info</wa:mls></a>
						    			</c:when>
		      			    			<c:otherwise>
			      			    			<a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" class="btn_more_info" title="${item.mainTitle}"><wa:mls>More info</wa:mls></a>
		      			    			</c:otherwise>
		      						</c:choose>
      							</div>
				 					...
				 					<a href="javascript:callLetter('${wa:numberToChar(indexLetter)}', 'map')" class="map"><span><wa:mls>Map :</wa:mls></span><img src="${static}/front/img/ml/btn_map_${wa:numberToChar(indexLetter)}.jpg" alt="Map ${wa:numberToChar(indexLetter)}"/></a>
				<br />
				<a href="javascript:callLetter('${wa:numberToChar(indexLetter)}', 'sv')" class="sv"><span><wa:mls>Street view :</wa:mls></span><img src="${static}/front/img/ml/btn_streetview.jpg" alt="Street view"/></a>
        ...
        <div class="scrollable" id="chained2" >
          <div class="items">
            ...
            <c:forEach items="${tickerList}" var="item" varStatus="loop">
	           <div>
	                <div class="ticker"><img src="${context}/static/front/img/ml/picto_news.jpg" alt="ticker"/>
			            <p>
			            	<a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>">${item.truncMainTitle}</a> 
	...
	<c:forEach var="item" items="${centerList}"  varStatus="loop">
		<c:if test="${(loop.count mod 2) ne 0}">
			...
			<div class="clr second_highlight_home float_left line_${item.color}">
		      <h3><c:choose>
		        	<c:when test="${item.group == 'HOTEL'}">
					   <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" ><span class="cutH2SmallTitle">${item.mainTitle}</span></a>
				    </c:when>
      			    <c:otherwise>
	      			    <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" ><span class="cutH2SmallTitle">${item.mainTitle}</span></a>
      			    </c:otherwise>
      			</c:choose>
      			<c:if test="${not empty item.periodFromDate}">
	          						<c:choose>
          	...
          	<c:when  test="${not empty item.mainSmallImage}">
	        	<c:choose>
		        	<c:when test="${item.group == 'HOTEL'}">
					    <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				        		<img src="${item.mainSmallImage}" alt="${item.mainTitle}" width="300"/>
				        </a>
				    </c:when>
      			    <c:otherwise>
	      			    <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
          ...
          <c:choose>
		        	<c:when test="${item.group == 'HOTEL'}">
					    <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				        		<img src="${context}/static/front/img/ml/default_content.jpg" alt="${item.mainTitle}" width="300"/>
				        </a>
				    </c:when>
      			    <c:otherwise>
	      			    <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
	        ...
	        <c:if test="${not empty item.URLWebsite && item.group != 'HOTEL'}">
		      ...
		      <div class="btn"> 
		      		<div class="more_info">
						...
						<c:if test="${item.contentType ne 'ContentArticle' and not empty item.latitude and item.latitude != 0.0 and not empty item.longitude and item.longitude != 0.0}">
				 <a href="javascript:callLetter('${wa:numberToChar(loop.count-1)}')" class="map" style="width: 55px;"><span><wa:mls>Map :</wa:mls></span><img src="${static}/front/img/ml/btn_map_${wa:numberToChar(loop.count-1)}.jpg" alt="Map ${wa:numberToChar(loop.count-1)}"/></a>
				 <script type="text/javascript">
          			addMarker(${item.longitude},${item.latitude},"${item.mainTitle}","${wa:numberToChar(loop.count-1)}");
          		</script>
				</c:if>  
				<c:set var="varTmp" value="\"" />
		      		<a href="http://www.addthis.com/bookmark.php?v=250&amp;username=wanabe" class="addthis_button share"  addthis:url="${config['URL']}${context}/front/content/detail/group/${item.group}/id/${item.id}.do" addthis:title="${item.mainTitle}" addthis:description="${fn:replace(wa:html2txt(item.mainAbstract_),varTmp,'&quot;')}"><img src="${static}/front/img/ml/btn_share.gif" alt="<wa:mls>Share</wa:mls>"/></a>
...
<c:forEach var="item" items="${bottomList}" varStatus="loop">    
    ...
    <c:choose>
          ...
          <c:choose>
		        	<c:when test="${item.group == 'HOTEL'}">
					    <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
				        		<img src="${context}/static/front/img/ml/default_content.jpg" alt="${item.mainTitle}" width="96"/>
				        </a>
				    </c:when>
      			    <c:otherwise>
	      			    <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" >
        ...
        <div class="txt">
          <h5><c:choose>
		        	<c:when test="${item.group == 'HOTEL'}">
					   <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" >${item.mainTitle}</a>
				    </c:when>
      			    <c:otherwise>
	      			    <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.mainTitle}" >${item.mainTitle}</a>
      			    </c:otherwise>
      			</c:choose></h5>
         <c:choose>
            	<c:when test="${item.contentType ne 'ContentArticle' && empty item.mainAbstract_}">
