...
<div class="first_highlight_home">
    ...
    <c:choose>
        <c:when test="${fn:length(mainList) > 1}">
            <div class="scrollable" id="chained">
        </c:when>
        <c:otherwise>
            <div class="scrollable" id="not_chained">
        </c:otherwise>
    </c:choose>
        <div class="items">
                        ...
                        <c:otherwise>
                            <div class="first_highlight_home_slide">
                                    ...
                                    <c:choose>
                                        <c:when test="${item.group == 'HOTEL'}">
                                            <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>"
                                               title="${item.mainTitle}"><span
                                                    class="cutH2Title">${item.mainTitle}</span></a>
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <div class="btn" style="border:0 !important;">
                                    </c:otherwise>
                                </c:choose>
                                    <div class="more_info">
                                        <c:choose>
                                            <c:when test="${item.group == 'HOTEL'}">
                                                <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>"
                                                   class="btn_more_info"
                                                   title="${item.mainTitle}"><wa:mls>More info</wa:mls></a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>"
                                                   class="btn_more_info"
                                    ...
                                    <c:if test="${item.contentType ne 'ContentArticle' and not empty item.latitude and item.latitude != 0.0 and not empty item.longitude and item.longitude != 0.0}">
                                        <a href="javascript:callLetter('${wa:numberToChar(indexLetter)}', 'map')"
                                           class="map"><span><wa:mls>Map :</wa:mls></span><img
                                                src="${static}/front/img/ml/btn_map_${wa:numberToChar(indexLetter)}.jpg"
    ...
    <c:if test="${fn:length(mainList) > 1}">
        <div class="navi">
                        ...
                        <div>
                            <div class="ticker"><img src="${context}/static/front/img/ml/picto_news.jpg" alt="ticker"/>
                                ...
                                <p>
                                    ...
                                    <strong><fmt:formatDate pattern="dd/MM/yyyy" value="${item.periodFromDate}"/> -
                                        <fmt:formatDate pattern="dd/MM/yyyy" value="${item.periodToDate}"/></strong>
                                </p>
        ...
        <c:otherwise>
            ...
            <div class="clr second_highlight_home float_left line_${item.color}">
                <h3><c:choose>
                    <c:when test="${item.group == 'HOTEL'}">
                        <a href="<wa:url URI="${site}/content/displayDetailBooking/group/${item.group}/id/${item.id}/name/${waParam.name}"/>"
                           title="${item.mainTitle}"><span class="cutH2SmallTitle">${item.mainTitle}</span></a>
                    </c:when>
                    <c:otherwise>
                        <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>"
                           title="${item.mainTitle}"><span class="cutH2SmallTitle">${item.mainTitle}</span></a>
                ...
                <div class="btn">
                    ...
                    <c:if test="${item.contentType ne 'ContentArticle' and not empty item.latitude and item.latitude != 0.0 and not empty item.longitude and item.longitude != 0.0}">
                        <a href="javascript:callLetter('${wa:numberToChar(loop.count-1)}')" class="map"
                           style="width: 55px;"><span><wa:mls>Map :</wa:mls></span><img
                    ...
                    <a href="http://www.addthis.com/bookmark.php?v=250&amp;username=wanabe" class="addthis_button share"
                       addthis:url="${config['URL']}${context}/front/content/detail/group/${item.group}/id/${item.id}.do"
                       addthis:title="${item.mainTitle}"
                       addthis:description="${fn:replace(wa:html2txt(item.mainAbstract_),varTmp,'&quot;')}"><img
                            src="${static}/front/img/ml/btn_share.gif" alt="<wa:mls>Share</wa:mls>"/></a>
        ...
        <c:otherwise>
            <div class="second_highlight_home float_right line_${item.color}">
                ...
                <div class="btn">
                    ...
                    </c:if> <a href="http://www.addthis.com/bookmark.php?v=250&amp;username=wanabe"
