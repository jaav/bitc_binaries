         ...
         <div id="pratical_information" class="location vcard">
	        ...
	        <c:if test="${contentType ne 'ContentArticle'  && contentGroupType ne 'ContentProduct' and not empty bean.latitude and bean.latitude != 0.0 and not empty bean.longitude and bean.longitude != 0.0}">
			 ...
			 <c:if test="${bean.group == 'GUIDED_TOUR' && bean.source == 'VoirEtDire'}">
			    ...
			    <a href="/bitc/front/tracking/link/desc/voiretdirebruxelles/cid/F0FDB0F455B48382.do" target="_blank"><img src="${static}/front/img/ml/btn_voiretdire.jpg" alt="voir et dire bruxelles" /></a>
			</c:if>
			 <c:if test="${not empty propTalk}">
					<a href="${propTalk}" target="_blank"><img class="btn_museumtalks" src="${static}/front/img/ml/btn_museumtalks.jpg" alt="museum talks" /></a>
				</c:if>
			  
			<c:if test="${bean.group == 'BnB' && bean.source == 'BednBrussels'}">
					<wa:mls>Pour réservation, cliquez ici</wa:mls><br /><br />
					<a href="${bean.mainUrl}" target="_blank"><img src="${static}/front/img/ml/btn_bednbrussels.jpg" alt="bed and brussels" /></a>
			</c:if>
