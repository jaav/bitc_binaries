      ...
      <c:forEach var="item" items="${list}" varStatus="loop">
        ...
        <a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" > 
          ...
          <div class="txt" id="txt_${item.id}">
            <h3 class="no_margin"><a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" title="${item.formatMainTitle}">${item.mainTitle}</a></h3>
            <c:choose>
          ...
          <div class="btn">
          		<div class="more_info"><a href="<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" class="btn_more_info" title="${item.formatMainTitle}"><wa:mls>More info</wa:mls></a></div>
				...
				<a href="http://www.addthis.com/bookmark.php?v=250&amp;username=wanabe" class="addthis_button share"  addthis:url="${config['URL']}<wa:url URI="${site}/content/displayDetail/group/${item.group}/id/${item.id}/name/${waParam.name}"/>" addthis:title="${item.formatMainTitle}" addthis:description="${fn:replace(wa:html2txt(item.mainAbstract_),varTmp,'&quot;')}"><img src="${context}/static/front/img/ml/btn_share.gif" alt="<wa:mls>Share</wa:mls>"/></a>
