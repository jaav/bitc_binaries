...
<p><wa:mls>Retrouvez ci-dessous d'autres idées de lieux ou d'événements à voir ou à faire à Bruxelles, qu'ils soient appréciés par d'autres utilisateurs qui ont choisi ce lieu, qu'ils soient dans le même quartier, du même profil ou de même type d'expérience.</wa:mls></p>
      ...
      <ul class="TabbedPanels">
        <c:if test="${not empty userLiked}"><li class="<c:if test="${tabbedPanelsTab eq 'UserAlsoLiked'}">TabbedPanelsTabSelected</c:if><c:if test="${tabbedPanelsTab ne 'UserAlsoLiked'}">TabbedPanelsTab</c:if>"><a href="<wa:url URI="${site}/content/${pageDetail }/group/${group }/id/${contentId}/name/${waParam.name}"/>?tabbedPanelsTab=UserAlsoLiked&property=none#associatedContent"><wa:mls>Users <br />also liked</wa:mls></a></li></c:if>
