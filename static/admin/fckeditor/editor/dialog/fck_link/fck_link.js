...
var sUri, sInnerHtml,sDescription ;
function getTrackUrl(protocol,uri,description){
	var oXML = new FCKXml() ;
	oXML.LoadUrl(FCKConfig.TrackLinkUrl+'?txtUrl='+uri+'&descUrl='+description+'&protocol='+protocol+'&uuid=' + new Date().getTime(), TrackLinkBack) ;
	
}
function TrackLinkBack(fckXml){
	var oNode = fckXml.SelectSingleNode( 'cryptUrl' );
	sUri = oNode.firstChild.nodeValue;
	Ok2();
}
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		case 'url' :
			...
			sDescription = GetE('descUrl').value ;
			...
			getTrackUrl(GetE('cmbLinkProtocol').value,sUri,sDescription);
...
function Ok2(){
	...
	oEditor.FCKDialog.OnDialogClose( window.parent ) ;
	return true ;
}
